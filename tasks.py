...
class AINewsLetterTask():
    def NewsFetcherTask(self,agent):
        return Task(
            description=f"Fetch the top AI news for the past 24 hours. The current time is {datetime.now()}.",
            agent=agent,
            async_execution=True,
            expected_output="""A list of top AI news story titles, urls and a brief summary
            for each story from the past 24 hours.
            Example output:
            [
    ...
    def NewsAnalyzerTask(self,agent,context):
        return Task(
            description='Analyze each news story and ensure there are atleast 5 well-formatted articles',
            agent=agent,
            context=context,
            async_execution=True,
